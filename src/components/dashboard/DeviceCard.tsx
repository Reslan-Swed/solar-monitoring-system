...
export const DeviceCard: React.FC<DeviceCardProps> = ({ device, onClick, onDelete, onRename, showMonitoringInfo, hoverOverlayText }) => {
  ...
  return (
    <motion.div
      ...
      className="bg-white rounded-2xl border border-slate-200 p-6 cursor-pointer hover:shadow-xl hover:shadow-slate-200 transition-all group flex flex-col h-full relative overflow-hidden"
    >
      {/* Subtle hover background effect */}
      <div className="absolute top-0 right-0 w-32 h-32 bg-amber-50 rounded-full -mr-16 -mt-16 opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-3xl -z-0" />
      ...
      <div className="flex items-start justify-between mb-4 relative z-10">
        ...
        <div className={cn(
          ...
          device.status === 'error' ? "bg-red-50 text-red-600" : "bg-slate-50 text-slate-600"
        )}>
      ...
      <div className="mb-4 relative z-10">
        <h3 className="font-bold text-slate-900 group-hover:text-amber-600 transition-colors">{device.name}</h3>
        <div className="flex flex-col gap-1 mt-1">
          <div className="flex items-center gap-1 text-slate-500 text-xs font-mono">
            <span className="font-bold uppercase text-[10px] text-slate-400">SN:</span>
            {device.deviceSn}
          </div>
        </div>
      </div>
      ...
      {showMonitoringInfo && (
        <div className="grid grid-cols-1 gap-3 mb-6 flex-grow relative z-10">
          <div className="flex justify-between items-center p-3 bg-slate-50 group-hover:bg-white border border-transparent group-hover:border-slate-100 rounded-xl transition-colors">
            <div className="flex items-center gap-2">
              <Sun className="w-3.5 h-3.5 text-amber-500" />
              <span className="text-[10px] font-bold text-slate-400 uppercase">PV Power</span>
            </div>
            <span className="text-sm font-bold text-slate-900">{currentPvPower} W</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-slate-50 group-hover:bg-white border border-transparent group-hover:border-slate-100 rounded-xl transition-colors">
            <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-bold text-slate-900">{currentBattery}%</span>
          </div>
          <div className="flex justify-between items-center p-3 bg-slate-50 group-hover:bg-white border border-transparent group-hover:border-slate-100 rounded-xl transition-colors">
            <div className="flex items-center gap-2">
            ...
            <span className="text-[10px] font-bold text-slate-600">{currentWorkMode}</span>
          </div>
        </div>
      )}

      {showMonitoringInfo && (
        <div className="flex items-center justify-end pt-4 border-t border-slate-100 mt-auto relative z-10">
          <div className="flex items-center gap-2 text-amber-600 text-sm font-bold transform translate-y-2 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
            Monitor Device
