...
export const AlertsList: React.FC<AlertsListProps> = ({ alerts, onMarkRead }) => {
  const severityStyles = {
    ...
    high: { icon: AlertOctagon, color: 'text-red-500', bg: 'bg-red-50', border: 'border-red-100' },
  };
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-slate-500">Real-time notifications about your solar infrastructure</p>
        </div>
        <button className="text-sm font-bold text-amber-600 hover:bg-amber-50 px-4 py-2 rounded-lg transition-all">
          Mark All as Read
        </button>
      </div>
      ...
      <div className="space-y-4">
        {alerts.length === 0 ? (
          <div className="bg-white border border-slate-200 rounded-2xl p-12 flex flex-col items-center text-center">
            ...
            <h3 className="font-bold text-slate-900">All systems operational</h3>
            <p className="text-slate-500 max-w-sm mt-1">No alerts found for the current period. We'll notify you if anything changes.</p>
          </div>
        ) : (
          alerts.map((alert, i) => {
            ...
            return (
              <motion.div
                ...
                key={alert.id}
                className={cn(
                  "bg-white border rounded-2xl p-5 flex items-start gap-4 transition-all hover:shadow-lg hover:shadow-slate-100",
                  style.border,
                  !alert.isRead && "ring-2 ring-amber-500/10"
                )}
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    <h4 className="font-bold text-slate-900 truncate">{alert.deviceName}</h4>
